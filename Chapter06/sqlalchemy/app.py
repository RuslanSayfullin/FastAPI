...
async def get_post(post: Post = Depends(get_posts_or_404)) -> Post:
    return post

@app.post(
    "/posts", response_model=schemas.PostRead, status_code=status.
HTTP_201_CREATED
)
...
async def update_post(
    ...
    post: Post = Depends(get_posts_or_404),
    session: AsyncSession = Depends(get_async_session),
) -> Post:
